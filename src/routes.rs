...
pub async fn index() -> NamedFile {
    let response = NamedFile::open("website/index.html").await.ok();
...
pub async fn signup() -> RawHtml<String> {
    let response = render("website/signup.html", None, None, None).unwrap();
    RawHtml(response)
}
...
pub async fn signup_data(user_data: Form<LoginData>, dm: &State<Arc<Mutex<UserManager>>>) -> Result<Redirect, RawHtml<String>> {
    ...
    match (*dm_unwrapped).add_new_user(user_data.to_user()) {
        ...
        Err(e) => {
            let response = render("website/signup.html", None, Some(&e), None).unwrap();
...
pub async fn login() -> RawHtml<String> {
    let response = render("website/login.html", None, None, None).unwrap();
    RawHtml(response)
}
...
pub async fn login_data(user_data: Form<LoginData>, dm: &State<Arc<Mutex<UserManager>>>, jar: &CookieJar<'_>) -> Result<Redirect, RawHtml<String>> {
    ...
    match (*dm_unwrapped).login_user(&current_user, jar) {
        Ok(_) => Ok(Redirect::to(uri!("/upload.html"))),
        Err(e) => Err(RawHtml(render("website/login.html", None, Some(&e), None).unwrap()))
    }
}

#[get("/")]
pub async fn upload(dm: &State<Arc<Mutex<UserManager>>>, jar: &CookieJar<'_>) -> RawHtml<String> {
    ...
    //render(user, file_path)


    let response = render("website/upload.html", Some(current_user), None, None).unwrap();
    RawHtml(response)
}
...
pub async fn upload_data(mut form: Form<Upload<'_>>, dm: &State<Arc<Mutex<UserManager>>>, jar: &CookieJar<'_>) -> RawHtml<String> {
    ...
    let response = render("website/upload.html", Some(&current_user), None, Some(&vec![success_message, "View your files here".to_string()])).unwrap();
    RawHtml(response)
}
...
pub async fn my_files(dm: &State<Arc<Mutex<UserManager>>>, jar: &CookieJar<'_>) -> RawHtml<String> {
    ...
    let response = render_myfiles("website/myfiles.html", current_user).unwrap();
    RawHtml(response)
}
